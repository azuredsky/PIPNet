...
def process_data_video(root_folder, fname, keypoints, target_size):
    image = cv2.imread(fname)
    image_height, image_width, _ = image.shape
    anno_x = [int(x[0]) for x in keypoints]
    anno_y = [int(x[1]) for x in keypoints]
    bbox_xmin = min(anno_x)
    bbox_ymin = min(anno_y)
    bbox_xmax = max(anno_x)
    bbox_ymax = max(anno_y)
    ...
    bbox_width = min(bbox_width, image_width - bbox_xmin - 1)
    bbox_height = min(bbox_height, image_height - bbox_ymin - 1)
    anno = [[(x - bbox_xmin) / bbox_width, (y - bbox_ymin) / bbox_height] for x, y in keypoints]
    ...
    #######head pose
    reprojectdst, euler_angle = get_head_pose(keypoints, image_crop)
    PRY = euler_angle.reshape([-1]).astype(np.float32) / 90.

    ######cla_label
    cla_label = np.zeros([4])
    if np.sqrt(np.square(anno_x[37] - anno_x[41]) +
               np.square(anno_y[37] - anno_y[41])) / target_size < 0.02 \
            or np.sqrt(np.square(anno_x[38] - anno_x[40]) +
                       np.square(anno_y[38] - anno_y[40])) / target_size < 0.02:
        cla_label[0] = 1
    if np.sqrt(np.square(anno_x[43] - anno_x[47]) +
               np.square(anno_y[43] - anno_y[47])) / target_size < 0.02 \
            or np.sqrt(np.square(anno_x[44] - anno_x[46]) +
                       np.square(anno_y[44] - anno_y[46])) / target_size < 0.02:
        cla_label[1] = 1
    if np.sqrt(np.square(anno_x[61] - anno_x[67]) +
               np.square(anno_y[61] - anno_y[67])) / target_size < 0.02 \
            or np.sqrt(np.square(anno_x[62] - anno_x[66]) +
                       np.square(anno_y[62] - anno_y[66])) / target_size < 0.02 \
            or np.sqrt(np.square(anno_x[63] - anno_x[65]) +
                       np.square(anno_y[63] - anno_y[65])) / target_size < 0.02:
        cla_label[2] = 1

    ### mouth open big   1 mean true
    if np.sqrt(np.square(anno_x[62] - anno_x[66]) +
               np.square(anno_y[62] - anno_y[66])) / target_size > 0.08:
        cla_label[3] = 1
...
def gen_data(root_folder, data_name, target_size):
    if not os.path.exists(os.path.join(root_folder, data_name, 'images_train')):
        os.makedirs(os.path.join(root_folder, data_name, 'images_train'))
    if not os.path.exists(os.path.join(root_folder, data_name, 'images_val')):
        os.makedirs(os.path.join(root_folder, data_name, 'images_val'))
    if not os.path.exists(os.path.join(root_folder, data_name, 'images_test')):
        os.makedirs(os.path.join(root_folder, data_name, 'images_test'))
    ...
    ################################################################################################################
        ...
        with open(os.path.join(root_folder, 'AFLW', 'test.txt'), 'w') as f:
            for index in test_indices:
                ...
                f.write('\n')
        gen_meanface(root_folder, data_name)
    elif data_name == 'data_video':
        ann_jsons = glob.glob(os.path.join(root_folder, "*.json"))
        for ann_json in ann_jsons:
            anno_name = ann_json.split('/')[-1]
            annos = {}
            with open(ann_json, 'r') as f:
                ann_list = json.load(f)
                for ann in ann_list:
                    fname = os.path.join(root_folder, 'tmp_crop_data_face_landmark_pytorch', ann['image_name'])
                    keypoints = ann['keypoints']
                    bbox = ann['bbox'][:4]
                    image_crop, anno = process_data_video(root_folder, fname, keypoints, target_size)
                    image_crop_name = os.path.join(root_folder, data_name, 'images_'+anno_name.replace('.json', ''), ann['image_name'].replace('/','_'))
                    # print(image_crop_name)
                    # cv2.imwrite(image_crop_name, image_crop)
                    annos[ann['image_name'].replace('/','_')] = anno
            anno_file = os.path.join(root_folder, data_name, anno_name.replace('.json', '_prob.txt'))
            with open(anno_file, 'w') as f:
                for image_crop_name, anno in annos.items():
                    f.write(image_crop_name + ' ')
                    for a in anno:
                        f.write(str(a) + ' ')
                    f.write('\n')
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        gen_data('/mnt/lustre02/jiangsu/aispeech/home/flc23/work/20_landmark/landmark-opt', data_name, 256)
